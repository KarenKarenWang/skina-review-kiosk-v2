...
export default function AdminClient({
  ...
  function switchLang(nextLang: "en" | "zh") {
    setLang(nextLang);
    router.push(`/admin?lang=${nextLang}&token=${encodeURIComponent(token)}`);
    router.refresh();
  }
    ...
    <main style={{ maxWidth: 860, margin: "40px auto", padding: 16 }}>
      ...
      <section style={{ marginTop: 24, ...box }}>
        ...
        >
          <input
            ...
            placeholder={
              lang === "zh"
                ? "输入一条中文评价模板"
                : "Enter one English review template"
            }
        ...
        <div style={{ marginTop: 8, fontSize: 12, opacity: 0.75 }}>
          Tip:{" "}
          {lang === "en"
            ? "Do not mix Chinese characters in English pool."
            : "中文池建议不要混入英文（避免误导）"}
        </div>
      </section>
      ...
      <section style={{ marginTop: 16, ...box }}>
        ...
        <form
          action={async (fd) => runAction(actions.bulkAdd, fd)}
          style={{ marginTop: 10 }}
        >
          <textarea
            name="bulk"
            rows={8}
            placeholder={
              lang === "zh" ? "每行一条中文文案（回车分行）" : "One per line (press Enter)"
            }
